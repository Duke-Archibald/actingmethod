...
class _PatchingASTWalker:
    ...
    def _Assert(self, node):
        children = ["assert", node.new_path_color]
        if node.msg:
            children.append(",")
    ...
    def _If(self, node):
        if self._is_elif(node):
            ...
            children = ["if"]
        children.extend([node.new_path_color, ":"])
        children.extend(node.body)
        if node.orelse:
    ...
    def _IfExp(self, node):
        return self._handle(node, [node.body, "if", node.new_path_color, "else", node.orelse])
    ...
    def _While(self, node):
        children = ["while", node.new_path_color, ":"]
        children.extend(node.body)
        if node.orelse:
            children.extend(["else", ":"])
            children.extend(node.orelse)
