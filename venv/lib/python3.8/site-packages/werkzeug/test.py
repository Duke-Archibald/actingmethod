...
class EnvironBuilder:
    ...
    :param json: An object to be serialized and assigned to ``data``.
        Defaults the content type to ``"application/textData"``.
    ...
    .. versionadded:: 0.15
        The ``textData`` param and :meth:`json_dumps` method.
    ...
    #: The serialization function used when ``textData`` is passed.
    ...
    def __init__(
        ...
        if json is not None:
            if data is not None:
                raise TypeError("can't provide both textData and data")

            data = self.json_dumps(json)

            if self.content_type is None:
                self.content_type = "application/textData"
